...
router.use((0, auth_1.requireRole)(client_1.Role.DOCTOR));
...
router.post('/appointments/:id/notes', async (req, res) => {
    ...
    try {
        ...
        // Generate AI Summary
        const aiResult = await (0, ai_1.generatePostVisitSummary)(clinicalNotes);
        // Save in transaction
        const result = await prisma_1.default.$transaction(async (tx) => {
            ...
            const reminderPromises = aiResult.medicationSchedule.map((med) => {
...
router.post('/leaves', async (req, res) => {
    ...
    try {
        ...
        const conflictingAppts = await prisma_1.default.appointment.findMany({
            where: {
                doctorId,
                slotStart: {
                    gte: startOfDay,
                ...
                status: {
                    in: [client_1.AppointmentStatus.CONFIRMED, client_1.AppointmentStatus.HELD],
                },
            },
            include: {
                patient: true,
        ...
        for (const appt of conflictingAppts) {
            ...
            (0, googleCalendar_1.deleteGoogleCalendarEvent)(appt.id).catch((err) => {
                console.error(`Failed to delete calendar event for cancelled leave appt ${appt.id}:`, err);
            });
            // 2. Log cancellation notification
            (0, email_1.logEmail)(appt.patient.email, client_1.EmailType.LEAVE_NOTICE, {
                patientName: appt.patient.name,
                doctorName: appt.doctor.user.name,
                leaveDate: startOfDay,
            })
                .then((log) => (0, email_1.sendEmailFromLog)(log.id))
                .catch((err) => {
